...
static int lvNewLabel(lua_State *L) {
    ...
    Class c = [LVUtil upvalueClass:L defaultClass:[LVLabel class]];
    {
        ...
        {
            print_stack(L);
            NEW_USERDATA(userData, View);// 创建一个LVUserDataInfo类型的变量userData
            print_stack(L);
            //LVUserDataInfo
...
+(int) lvClassDefine:(lua_State *)L globalName:(NSString*) globalName{
    ...
    //把lvNewLabel推入栈变为lua中的闭包，clas为upvalue，Label为其在lua中的名字
    [LVUtil reg:L clas:self cfunc:lvNewLabel globalName:globalName defaultName:@"Label"]; //注册lvNewLabel为lua方法Label()
    print_stack(L);
    ...
    // 创建Label类的方法列表
    lv_createClassMetaTable(L, META_TABLE_UILabel); //META_TABLE_UILabel在注册表中
    
    print_stack(L);
    //把函数都注册到META_TABLE_UILabel中
