...
void lv_createClassMetaTable(lua_State* L , const char* name ){
    luaL_newmetatable(L, name ); //创建元表 其实就是创建在注册表里面的表
...
static int lv_setUDataLuatable (lua_State *L, int index) {
    if( lua_type(L, index)==LUA_TUSERDATA ){ //判断lua type类型
        lua_setfenv(L, index); //从堆栈中弹出一个表，并将其设置为给定索引处的值的新环境
        return 1;
    }
    return 0;
}
...
int lv_createUDataLuatable (lua_State *L, int index){
     ...
     */
    //print_stack(L);
    lua_pushvalue(L, index); //把userdata拷贝到栈顶
    lua_createtable(L, 8, 0); //创建一个表到栈顶
    lv_setUDataLuatable(L, -2); //从堆栈中弹出一个表，并将其设置为给定索引处的值的新环境
    lua_pop(L, 1); //把刚才lua_pushvalue的值pop掉
    return 1;
}
...
+(void) reg:(lua_State*)L clas:(id) c cfunc:(lua_CFunction) cfunc globalName:(NSString*)globalName defaultName:(NSString*) defaultName{
    if( defaultName || globalName ) {
        lua_checkstack(L, 12); //lua的栈不是自动扩展的，需要定时检查一下
        NSString* className = NSStringFromClass(c);
      //  print_stack(L);
        lua_pushstring(L, className.UTF8String); //字符串推入栈
        lua_pushcclosure(L, cfunc, 1); //压入闭包,1代表1个upvalue
      //  print_stack(L);
        lua_setglobal(L, globalName ? globalName.UTF8String : defaultName.UTF8String );//把栈顶的数据变为lua变量，第二个参数为变量名
    }
}
